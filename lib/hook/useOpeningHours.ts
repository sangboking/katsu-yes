...
import { getToday, getSortedOpeningHours } from '@/lib/time';
import type { OpeningHours } from './useKatsuPlaceDetail';

export const useOpeningHours = (openingHours: OpeningHours | undefined) => {
  const today = useMemo(() => getToday(), []);

  const sortedOpeningHours = useMemo(() => getSortedOpeningHours(openingHours), [openingHours]);
  ...
  return { today, sortedOpeningHours, todayOpeningHours };
};
